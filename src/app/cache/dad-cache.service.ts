...
export class DadCacheService {
  ...
  constructor() { }
  Save = <T>(key: string, data: any, refreshMs?: number | null | undefined): IDadCache<T> => {
    let cache = {
      Data: data,
      CreatedUnix: this.now(),
      ExpiresUnix: refreshMs ? this.now() + refreshMs : refreshMs
    } as IDadCache<T>
    localStorage.setItem(this.getFullKey(key), JSON.stringify(cache))
    let keys = this.getMeta()
    if (!keys.includes(key)) keys.push(this.getFullKey(key))
    this.setMeta(keys)
    return cache
  }
  ...
  Get<T>(key: string, clearBeforeUnix?: number | undefined): IDadCache<T> | null {
    ...
    if (
      ...
      || (clearBeforeUnix && clearBeforeUnix > cache.CreatedUnix)
      ) {
      this.Clear(key)
      return null
    }
