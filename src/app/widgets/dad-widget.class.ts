...
export class DadWidget {
    ...
    constructor(public Data: IDadWidget, public Parent?: DadWidget, public Root?: DadWidget) {
        if (Data?.Form) {
            let formGroup = {} as { [prop: string]: FormControl }
            for(let fieldId of Data.Form){
                let field = this.Dictionary[fieldId]
                if(!field || !field.Data?.Key || !field.Field) {
                    console.error('Invalid Field on Form', this.Id, fieldId)
                    continue
                }
                formGroup[field.Data.Key] = field.Field
            }
